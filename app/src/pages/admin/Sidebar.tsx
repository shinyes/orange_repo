...
import type { ProblemSummary, ProblemType, TagCount, TagNode } from '@/api/types'
...
// 第一栏：标签筛选（搜索 / 类型 / 标签树）。
// 说明（路由化适配）：原 web 的栏头（品牌/角色/域管理·空间管理入口/设置/备份/登出）已上移到
// /admin 顶层 AdminLayout 顶栏；本栏只保留仓库筛选本体（global_admin 未选域时提示选域）。
export function TagFilterColumn() {
...
// （原为 TagFilterColumn 内部部件；路由化适配后供 AdminLayout 顶栏复用。）
