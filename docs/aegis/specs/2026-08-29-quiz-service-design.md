...
修订：2026-09-02a —— Docker 双服务部署：同一镜像内构建 OrangeOJ + quiz 两个二进制，compose 两容器共享数据卷；CI 发布流程不变（单 tag 同时交付两服务）；原「不动 Docker 流程」非目标解除
...
**非目标（第一阶段）**：编程题、多选/不定项、历史练习统计报表、题库本身编辑（仍在主站完成）。
