...
修订：2026-08-22h（v1.8.0）——导入入口拆分（题目栏仅题目，题册栏导入训练/练习）+ 题册行紧凑化与下载菜单 + 训练/练习默认查看模式 + 条目点击题目预览浮窗
修订：2026-08-22i（v1.9.0）——题目多选删除与 Shift 范围选择 + 题册导入入口合并 + 训练导入无章节结构时自动「未分组」收纳全部题目 + 导入 auto 自动识别与文件名命名 + 多包批量导入 + mode 参数规范化
修订：2026-08-22j（v1.9.1）——题目预览浮窗支持编辑模式 + 题册导入免选模式（固定自动识别）+ 删除目录三选（连同题册/移到顶层/取消） + 题册行更多菜单增加删除 + 删除题册级联删除仅被其引用的题目
...
修订：2026-08-22n（v1.9.5）——训练编辑模式章节直接文本框 + 修复题目图片静态路由（/api/uploads） + 设置对话框整合修改密码/清理未关联图片 + 标签列固定区不随滚动消失 + 单选题选项支持 Markdown/KaTeX 渲染
修订：2026-09-02b —— 账号统一（见刷题服务规格 §7.1）：主站认证改用共享账号库（quiz.db 的 users/sessions），登录改为用户名+密码且限 admin 角色，旧 settings 密码哈希自动迁移为 admin 账号

## 1. 目标与边界

构建题库管理 Web 应用 **OrangeOJ**：
...
删除语义（v1.1.0 起；级联删题 v1.9.1）：~~删除目录~~（功能移除）；删除题目 → 级联清理 training/practice 条目；删除训练/练习 → 级联章节与条目，**并将仅被该题册引用的题目一并删除（被其他训练/练习条目引用的题目保留）**；目录「连同题册删除」同样应用此级联规则；「移到顶层」不删任何题目。
...
PATCH /api/booklet-directories/:id（重命名 {name}）；DELETE /api/booklet-directories/:id[?deleteBooklets=true]
     删除语义（v1.9.0）：前端三选——删除目录时提示是否连同样内题册删除；
     保留则题册移到顶层；直接子目录始终上移一层。
...
DELETE /api/booklet-directories/:id[?deleteBooklets=true] → 204
     （v1.9.0）删除目录前由前端三选：连同样内题册一并删除（deleteBooklets=true），
     或保留题册并移到顶层；直接子目录始终上移一层。
...
导入语义：mode=problems|training|practice|auto（大小写/空白自动规范化，非法值返回 400）。mode=training 时用 meta.title/tags（缺省时用**上传文件名**兜底，再缺省「导入的训练」）建训练并按下标映射章节条目；**若 ZIP 未提供章节结构（缺 trainingPlan.json 或 chapters 为空），自动创建「未分组」章节收纳全部题目**；mode=practice 按数组顺序建平铺练习（缺省名同规则）；**mode=auto 按内容自动识别：含章节结构→训练，否则→练习**（v1.9.0）。前端导入支持一次多选多个 ZIP 包，逐个导入并汇总提示；每个包名称取元数据标题，缺失时用文件名。
...
- 中栏：题目列表（操作区 = 「+题目」/ 导入（**仅题目**）/ 导出 / 题册栏开关）；「题册」按钮展开**题册栏**（v1.6.0 起位于题目栏右侧，紧邻详情区）
- **题册栏**（v1.6.0；目录拖拽 v1.7.0；导入与紧凑行 v1.8.0；v1.9.0 多变）：标题行 = 「题册」计数 + **「+题册」下拉**（新建训练/新建练习/新建目录 + **导入题册**（v1.9.0 起固定自动识别、无需选择模式、支持多包批量），落入当前选中目录）+ 搜索；树体 = 可嵌套目录（点击行展开/收起并选中为新建落点，hover ⋮ 菜单：新建子目录/重命名/删除——删除时三选：连同题册删除 / 题册移到顶层 / 取消；子目录上移一层；**拖动目录行**：上/下边缘=调整同级顺序，中间=改为目标目录的子目录，底部投放区=移到顶层）+ 训练/练习题册行（**紧凑单行**：图标+名称+题数，与题目栏行高一致；hover ⋮ 菜单提供**下载（导出 ZIP）**；拖拽可移入目录或根区域）
- 右栏：空态统计页 / 题目详情（标题、类型徽标、标签、时限；Tabs 题面|答案|题解|编辑）/ 训练详情（章节+条目排序增删；**默认查看模式**，铅笔切换编辑）/ 练习详情（平铺条目，拖拽排序；**默认查看模式**）；训练/练习条目**点击弹出题目预览浮窗**（查看/编辑双模式，保存后回到查看），浮窗编辑与详情页共用编辑器
