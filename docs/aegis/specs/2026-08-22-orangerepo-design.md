...
- **题册栏**（v1.6.0；目录拖拽 v1.7.0；导入与紧凑行 v1.8.0）：标题行 = 「题册」计数 + **「+题册」下拉**（新建训练/新建练习/新建目录 + **导入训练/导入练习**，落入当前选中目录）+ 搜索；树体 = 可嵌套目录（点击行展开/收起并选中为新建落点，hover ⋮ 菜单：新建子目录/重命名/删除——删除时子目录与归属题册上移一层；**拖动目录行**：上/下边缘=调整同级顺序，中间=改为目标目录的子目录，底部投放区=移到顶层）+ 训练/练习题册行（**紧凑单行**：图标+名称+题数，与题目栏行高一致；hover ⋮ 菜单提供**下载（导出 ZIP）**；拖拽可移入目录或根区域）
- 右栏：空态统计页 / 题目详情（标题、类型徽标、标签、时限；Tabs 题面|答案|题解|编辑）/ 训练详情（章节+条目排序增删；**默认查看模式**，铅笔切换编辑）/ 练习详情（平铺条目，拖拽排序；**默认查看模式**）；训练/练习条目**点击弹出题目预览浮窗**（题面/答案/题解，查看与编辑模式均可）
